...
class _LocalCollection:
    ...
    def _documents(self) -> list[dict]:
        return self._database._collections.setdefault(
            self._name,
            [],
        )
...
async def connect() -> Any:
    """
    Connect to the permanent MongoDB database.

    MongoDB is the authoritative persistent storage for hosted Telegram
    sessions and application data.

    GitHub Actions runners are temporary. Therefore production startup
    MUST NOT fall back to local JSON storage.

    If MongoDB is temporarily unavailable, this function retries several
    times. If MongoDB still cannot be reached, startup is aborted instead
    of allowing the application to run against an empty local database.

    This protects hosted Telegram sessions from appearing to be lost after
    a GitHub Actions runner replacement or 5h45 handoff.
    """
    global _client, _db

    # A persistent MongoDB connection is mandatory.
    if not MONGO_URI:
        _client = None
        _db = None
        ...
        raise RuntimeError(
            ...
            "Persistent MongoDB storage is required "
            "for hosted sessions."
        )
    ...
    for attempt in range(
        ...
        try:
            ...
            # ─────────────────────────────────────────────────────────────
            # SetGroup / Saved Messages bridge
            # ─────────────────────────────────────────────────────────────
            ...
            # Only mark the database as ready AFTER ping and all indexes
            # have succeeded.
            _client = client
            _db = database
            ...
            if attempt < max_attempts:
                ...
                raise RuntimeError(
                    "MongoDB could not be reached after "
                    f"{max_attempts} attempts. "
                    "Startup aborted to protect persistent "
                    "hosted sessions."
                ) from exc
...
async def get_target_mappings(
    ...
    cursor = get_db().target_mappings.find(
        query,
        {"_id": 0},
    )
...
async def upsert_target_mapping(
    ...
    existing = await get_target_mapping(
        user_id,
        group_chat_id,
        target_user_id,
    )
...
async def remove_all_target_mappings(
    user_id: int,
) -> int:
    """Remove every permanent group-to-target mapping for one bot user."""
    result = await get_db().target_mappings.delete_many(
        {"user_id": user_id}
    )
...
async def get_latest_mapped_target_message(
    ...
    mappings = await get_target_mappings(
        user_id
    )
...
async def get_active_group(
    ...
    return (
        user.get("active_group")
        if user
        else None
    )


async def clear_active_group(
    user_id: int,
) -> None:
...
async def get_setgroup_mapping(
    user_id: int,
    saved_msg_id: int,
) -> dict | None:
    """
    ...
    return {
        "original_chat_id": doc.get(
            "original_chat_id",
            doc.get("group_chat_id"),
        ),
        "original_message_id": doc.get(
            "original_message_id",
            doc.get("group_msg_id"),
        ),
...
async def get_setgroup_mappings(
    user_id: int,
) -> list[dict]:
...
async def clear_monitoring_data(
    ...
    await get_db().group_messages.delete_many(
        target_filter
        | (
            {
    ...
    await get_db().setgroup_map.delete_many(
        mapping_filter
    )
...
async def get_all_active_users() -> list[dict]:
    """
    Return every user that has an active persistent Telegram session.

    This is used when a new GitHub Actions runner starts so all previously
    hosted accounts can be restored from MongoDB automatically.
    """
