...
async def register_commands():
    ...
    async def magisk_handler(event):
        ...
        try:
            ...
            await status.edit(f"🎭 **FLEX FUCKER USERBOT Magisk**\n\n"
                              f"{releases}\n"
                              f"✅ **Success!**")
    ...
    async def device_handler(event):
        try:
            ...
            if results := data.get(codename):
                ...
                await status.edit(f"🎭 **FLEX FUCKER USERBOT Device Search**\n\n"
    ...
    async def codename_handler(event):
        try:
            ...
            if results:
                ...
                await status.edit(f"🎭 **FLEX FUCKER USERBOT Codename Search**\n\n"
                                  f"{reply_text}"
                                  f"✅ **Success!**")
            else:
    ...
    async def twrp_handler(event):
        try:
            ...
            await status.edit(f"🎭 **FLEX FUCKER USERBOT TWRP Finder**\n\n"
                              f"📱 **Device:** `{device}`\n"
                              f"💾 **File:** `{dl_file}`\n"
                              f"📦 **Size:** `{size}`\n"
                              f"📅 **Date:** `{date}`\n\n"
