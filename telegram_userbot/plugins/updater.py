...
# ──────────────────────────────────────────────────────────────
_upstream = urlparse(Config.UPSTREAM_REPO.rstrip("/"))
_upstream_parts = [part for part in _upstream.path.split("/") if part]
GITHUB_OWNER  = _upstream_parts[0] if len(_upstream_parts) >= 2 else "shfhshddhd"
GITHUB_REPO   = _upstream_parts[1].removesuffix(".git") if len(_upstream_parts) >= 2 else "Tohid0.2"
